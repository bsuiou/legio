...
const EndlessCampaign = {
    ...
    renderCampaignSetupUI() {
        ...
        const mercHTML = this._mercenaries.length > 0 ? `
            <div class="campaign-veterans" style="margin-top:8px;">
                <h3 style="margin-bottom:8px; font-variant:small-caps; color:#d4a040;">Mercenaries (FREE)</h3>
                ${this._mercenaries.map((m, i) => {
                    const tc = TYPE_CONFIG[m.type]; const sc = SIZE_CONFIG[m.size];
                    return `<div class="vet-option merc-item"><span class="merc-tag">Merc.</span> ${tc.label} ${sc.label} <span class="merc-free">FREE</span></div>`;
                }).join('')}
            </div>
        ` : '';

        const battleNum = this.battleNumber + 1;
        container.innerHTML = `
            <div class="menu-content">
                ...
                <div class="unit-options" id="unitOptions">
                    ${options.map(opt => `
                        <div class="unit-option" data-type="${opt.type}" data-size="${opt.size}">
                            ${unitSymbolHTML(opt.type, opt.size)}
                            <span class="label">${opt.label}</span>
                            <span class="cost">${opt.cost}</span>
                        </div>
                    `).join('')}
                </div>
                <div class="selected-units" id="selectedUnits">
                    <h3>Your Legion</h3>
                    <div id="unitList"></div>
                </div>
                <div class="setup-buttons">
                    <button id="btnReady" class="menu-btn">Ready for Battle</button>
                    <button id="btnBackSetup" class="menu-btn small">Back</button>
                </div>
            </div>
        `;

        this._bindSetupEvents(budget, node);
    },

    _bindSetupEvents(budget, node) {
        const updateUI = () => {
            const listEl = document.getElementById('unitList');
            const pointsEl = document.getElementById('pointsLeft');
            if (!listEl || !pointsEl) return;
            pointsEl.textContent = Army.remaining;

            const nonMerc = Army.playerUnits.filter(u => !u._isMercenary);
            listEl.innerHTML = nonMerc.map((u, i) => {
                const tc = TYPE_CONFIG[u.type]; const sc = SIZE_CONFIG[u.size];
                const vet = u._veteranId ? '\u2605 ' : '';
                const mercLabel = u._isMercenary ? '<span class="merc-tag">Merc</span> ' : '';
                return `<div class="selected-unit">${mercLabel}${vet}${tc.label} ${sc.label} <button class="remove-btn" data-idx="${i}">\u00d7</button></div>`;
            }).join('');

            listEl.querySelectorAll('.remove-btn').forEach(btn => {
                btn.addEventListener('click', () => {
                    const idx = parseInt(btn.dataset.idx);
                    const nonMercUnits = Army.playerUnits.filter(u => !u._isMercenary);
                    const unit = nonMercUnits[idx];
                    if (!unit) return;
                    if (unit._veteranId) {
                        const vet = this.veteranRoster.find(v => v.id === unit._veteranId);
                        if (vet) vet.deployed = false;
                    }
                    Army.remaining += SIZE_CONFIG[unit.size].strength;
                    Army.playerUnits = Army.playerUnits.filter(u => u !== unit);
                    updateUI();
                });
            });
        };
        ...
        document.querySelectorAll('.unit-option').forEach(opt => {
            opt.addEventListener('click', () => {
                ...
                const cost = SIZE_CONFIG[size].strength;
                if (Army.remaining < cost) return;
                const unit = new Unit(type, size, 'player');
                Army.remaining -= cost;
                Army.playerUnits.push(unit);
                updateUI();
            });
        ...
        // Veteran options
        document.querySelectorAll('.vet-option').forEach(opt => {
            opt.addEventListener('click', () => {
                const vetId = opt.dataset.vetId;
                if (!vetId) return;
                const vet = this.veteranRoster.find(v => v.id === vetId);
                if (!vet || vet.deployed) return;
                if (Army.remaining < vet.cost) return;
                const unit = new Unit(vet.type, vet.size, 'player');
                unit._veteranId = vet.id;
                unit.veteran = true;
                vet.deployed = true;
                Army.remaining -= vet.cost;
                Army.playerUnits.push(unit);
                updateUI();
            });
        });
